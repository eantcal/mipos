...
typedef enum
{
    ...
    CAN_Page_Config =
      ((uint8_t)6), /*!< CAN Configuration control/status reg page*/
...
typedef enum
{
    ...
    CAN_ModeStatus_Success =
      !CAN_ModeStatus_Failed /*!< CAN entering the specific mode Succeed */
...
typedef enum
{
    CAN_MasterCtrl_AllDisabled =
      ((uint8_t)0x00), /*!<  CAN  ALL Master Control Option are DISABLED */
    CAN_MasterCtrl_AllEnabled =
      ((uint8_t)0xFC), /*!<  CAN  ALL Master Control Option are DISABLED */
    CAN_MasterCtrl_TimeTriggerCOMMode =
      ((uint8_t)0x80), /*!<  CAN Time Triggered Communication mode ENABLED */
    CAN_MasterCtrl_AutoBusOffManagement =
      ((uint8_t)0x40), /*!<  CAN Auto Bus Off Management ENABLED */
    CAN_MasterCtrl_AutoWakeUpMode =
      ((uint8_t)0x20), /*!<  CAN Automatic WakeUp Mode ENABLED , sleep mode is
                          left automatically by hardware  */
    CAN_MasterCtrl_NoAutoReTx =
      ((uint8_t)0x10), /*!<  CAN Non Automatic Retransmission ENABLED, MSG  will
                          be transmitted only once */
    CAN_MasterCtrl_RxFifoLockedMode =
      ((uint8_t)0x08), /*!<  CAN Receive FIFO Locked  against overrun ENABLED */
    CAN_MasterCtrl_TxFifoPriority =
      ((uint8_t)0x04) /*!<  CAN Transmit FIFO Priority  driven by the request
                         order (not by the identifier of the MSG) */
...
typedef enum
{
    ...
    CAN_FilterMode_IdMask_IdList =
      ((uint8_t)0x10), /*!< Id/Mask mode First and IdList mode second */
    CAN_FilterMode_IdList_IdMask =
      ((uint8_t)0x11), /*!< IdList mode First and IdMask mode second */
...
typedef enum
{
    ...
    CAN_TxStatus_NoMailBox =
      ((uint8_t)0xF4), /*!< CAN cell did not provide an empty mailbox */
    CAN_TxStatus_MailBoxEmpty = ((uint8_t)0xF5), /*!< CAN Tx mailbox is Empty */
    CAN_TxStatus_MailBox0Ok =
      ((uint8_t)0x00), /*!< CAN transmission succeeded by mail box 1*/
    CAN_TxStatus_MailBox1Ok =
      ((uint8_t)0x01), /*!< CAN transmission succeeded by mail box 2*/
    CAN_TxStatus_MailBox2Ok =
      ((uint8_t)0x05) /*!< CAN transmission succeeded by mail box 3*/
...
typedef enum
{
    /* if the flag is 0x3XXX, it means that it can be got (CAN_GetFlagStatus)
       and Cleared (CAN_ClearFlag) */
    /* if the flag is 0x1XXX, it means that it can only be got
       (CAN_GetFlagStatus)  */
    /*Transmit Flags*/
...
typedef enum
{
    CAN_ST7Compatibility_Enable =
      ((uint8_t)0x00), /*!< CAN is compatible with ST7 beCAN (only 2 mailboxes
                          are available)*/
    CAN_ST7Compatibility_Disable =
      ((uint8_t)0x10) /*!< CAN is not compatible with ST7 beCAN ( 3 mailboxes
                         are available)*/
 ...
 */
/**
 * @brief  Macro used by the assert function in order to check the CAN ST7
 * Compatibility parameters.
 */
#define IS_CAN_ST7_COMPATIBILITY_OK(STATE)                                     \
    (((STATE) == CAN_ST7Compatibility_Enable) ||                               \
     ((STATE) == CAN_ST7Compatibility_Disable))
/**
 * @brief  Macro used by the assert function in order to check  CAN operating
 * mode.
 */
...
/**
 * @brief  Macro used by the assert function in order to check CAN Time
 * Triggered Communication mode.
 */
#define IS_CAN_MASTERCTRL_OK(MODE)                                             \
    (((MODE) == CAN_MasterCtrl_AllDisabled) ||                                 \
     (((MODE) <= CAN_MasterCtrl_AllEnabled) &&                                 \
      ((MODE) >= CAN_MasterCtrl_TxFifoPriority)))
/**
 * @brief  Macro used by the assert function in order to check   CAN  mode
 * options .
 */
...
/**
 * @brief  Macro used by the assert function in order to check the CAN
 * synchronisation jump width (SJW).
 */
#define IS_CAN_SYNJUMPWIDTH_OK(SJW)                                            \
    (((SJW) == CAN_SynJumpWidth_1TimeQuantum) ||                               \
     ((SJW) == CAN_SynJumpWidth_2TimeQuantum) ||                               \
     ((SJW) == CAN_SynJumpWidth_3TimeQuantum) ||                               \
     ((SJW) == CAN_SynJumpWidth_4TimeQuantum))
/**
 * @brief  Macro used by the assert function in order to check time quantum in
 * bit segment 1 .
 */
 ...
 * bit segment 2.
 */
#define IS_CAN_BITSEG2_OK(BS2)                                                 \
    ((((BS2) >= CAN_BitSeg2_2TimeQuantum) &&                                   \
      ((BS2) <= CAN_BitSeg2_8TimeQuantum)) ||                                  \
     ((BS2) == CAN_BitSeg2_1TimeQuantum))
/**
 * @brief  Macro used by the assert function in order to check CAN clock
 * prescaler.
 */
#define IS_CAN_PRESCALER_OK(PRESCALER)                                         \
    (((PRESCALER) >= 1) && ((PRESCALER) <= 64))
/**
 * @brief  Macro used by the assert function in order to check CAN filter
 * number.
 */
#define IS_CAN_FILTER_NUMBER_OK(NUMBER)                                        \
    (((NUMBER) == CAN_FilterNumber_0) || ((NUMBER) == CAN_FilterNumber_1) ||   \
     ((NUMBER) == CAN_FilterNumber_2) || ((NUMBER) == CAN_FilterNumber_3) ||   \
     ((NUMBER) == CAN_FilterNumber_4) || ((NUMBER) == CAN_FilterNumber_5))
/**
...
#define IS_CAN_TRANSMITMAILBOX_OK(TRANSMITMAILBOX)                             \
    (((TRANSMITMAILBOX) == CAN_TransmitMailBox_0) ||                           \
     ((TRANSMITMAILBOX) == CAN_TransmitMailBox_1) ||                           \
     ((TRANSMITMAILBOX) == CAN_TransmitMailBox_2))
/**
 * @brief  Macro used by the assert function in order to check the Standard ID
 * to be sent.
 */
#define IS_CAN_STDID_OK(STDID) ((STDID) <= ((uint16_t)CAN_STDID_SIZE))
/**
 * @brief  Macro used by the assert function in order to check the Extended ID
 * to be sent.
 */
#define IS_CAN_EXTID_OK(EXTID) ((EXTID) <= ((uint32_t)CAN_EXTID_SIZE))
/**
 * @brief  Macro used by the assert function in order to check the DLC to be
 * sent.
 */
#define IS_CAN_DLC_OK(DLC) ((DLC) <= CAN_DLC_MAX)
/**
 * @brief  Macro used by the assert function in order to check the type of the
 * ID to be sent.
 */
#define IS_CAN_IDTYPE_OK(IDTYPE)                                               \
    (((IDTYPE) == CAN_Id_Standard) || ((IDTYPE) == CAN_Id_Extended))
/**
 * @brief  Macro used by the assert function in order to check CAN  transmission
 * Frame Type.
 */
#define IS_CAN_RTR_OK(RTR)                                                     \
    (((RTR) == CAN_RTR_Data) || ((RTR) == CAN_RTR_Remote))

/**
 * @brief  Macro used by the assert function in order to check  CAN flags which
 * can be got by @ref CAN_GetFlagStatus
 */
#define IS_CAN_FLAG_STATUS_OK(FLAG)                                            \
    (((FLAG) == CAN_FLAG_RQCP0) || ((FLAG) == CAN_FLAG_RQCP1) ||               \
...
/**
 * @brief  Macro used by the assert function in order to check  CAN flags which
 * can be cleared by @ref CAN_ClearFlag
 */
 ...
 */
#define CAN_IT_CONFIG_MASK                                                     \
    ~(uint16_t)(CAN_IT_TME | CAN_IT_FMP | CAN_IT_FF | CAN_IT_FOV |             \
                CAN_IT_WKU | CAN_IT_EWG | CAN_IT_EPV | CAN_IT_BOF |            \
                CAN_IT_LEC | CAN_IT_ERR)
#define IS_CAN_IT_CONFIG_OK(IT)                                                \
    (((IT) != 0x0000) &&                                                       \
     ((uint16_t)((uint16_t)(IT) & (uint16_t)CAN_IT_CONFIG_MASK) == 0x0000))
/**
 * @brief  Macro used by the assert function in order to check the  CAN status
 * interrupts.
 */
#define IS_CAN_IT_STATUS_OK(IT)                                                \
    (((IT) == CAN_IT_TME) || ((IT) == CAN_IT_FMP) || ((IT) == CAN_IT_FF) ||    \
     ((IT) == CAN_IT_FOV) || ((IT) == CAN_IT_WKU) || ((IT) == CAN_IT_ERR) ||   \
     ((IT) == CAN_IT_EWG) || ((IT) == CAN_IT_EPV) || ((IT) == CAN_IT_BOF) ||   \
     ((IT) == CAN_IT_LEC))
/**
 * @brief  Macro used by the assert function in order to check the  CAN Pending
 * bit interrupts.
 */
#define IS_CAN_IT_PENDING_BIT_OK(IT)                                           \
    (((IT) == CAN_IT_TME) || ((IT) == CAN_IT_FF) || ((IT) == CAN_IT_FOV) ||    \
     ((IT) == CAN_IT_WKU) || ((IT) == CAN_IT_ERR) || ((IT) == CAN_IT_EWG) ||   \
...
CAN_TxStatus_TypeDef CAN_TransmitStatus(
  CAN_TransmitMailBox_TypeDef CAN_TransmitMailbox);
...
CAN_ModeStatus_TypeDef CAN_OperatingModeRequest(
  CAN_OperatingMode_TypeDef CAN_OperatingMode);
