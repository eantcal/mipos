 ...
 * @brief   This file contains all functions prototype and macros for the SPI
 *peripheral.
 ******************************************************************************
 * @attention
 *
...
typedef enum
{
    SPI_DATADIRECTION_2LINES_FULLDUPLEX =
      (uint8_t)0x00, /*!< 2-line uni-directional data mode enable */
    SPI_DATADIRECTION_2LINES_RXONLY =
      (uint8_t)0x04, /*!< Receiver only in 2 line uni-directional data mode */
    SPI_DATADIRECTION_1LINE_RX =
      (uint8_t)0x80, /*!< Receiver only in 1 line bi-directional data mode */
    SPI_DATADIRECTION_1LINE_TX =
      (uint8_t)0xC0 /*!< Transmit only in 1 line bi-directional data mode */
...
{
    SPI_DIRECTION_RX =
      (uint8_t)0x00, /*!< Selects Rx receive direction in bi-directional mode */
    SPI_DIRECTION_TX = (uint8_t)0x01 /*!< Selects Tx transmission direction in
                                        bi-directional mode */
...
typedef enum
{
    SPI_BAUDRATEPRESCALER_2 =
      (uint8_t)0x00, /*!< SPI frequency = frequency(CPU)/2 */
    SPI_BAUDRATEPRESCALER_4 =
      (uint8_t)0x08, /*!< SPI frequency = frequency(CPU)/4 */
    SPI_BAUDRATEPRESCALER_8 =
      (uint8_t)0x10, /*!< SPI frequency = frequency(CPU)/8 */
    SPI_BAUDRATEPRESCALER_16 =
      (uint8_t)0x18, /*!< SPI frequency = frequency(CPU)/16 */
    SPI_BAUDRATEPRESCALER_32 =
      (uint8_t)0x20, /*!< SPI frequency = frequency(CPU)/32 */
    SPI_BAUDRATEPRESCALER_64 =
      (uint8_t)0x28, /*!< SPI frequency = frequency(CPU)/64 */
    SPI_BAUDRATEPRESCALER_128 =
      (uint8_t)0x30, /*!< SPI frequency = frequency(CPU)/128 */
    SPI_BAUDRATEPRESCALER_256 =
      (uint8_t)0x38 /*!< SPI frequency = frequency(CPU)/256 */
...
typedef enum
{
    SPI_CLOCKPHASE_1EDGE = (uint8_t)0x00, /*!< The first clock transition is the
                                             first data capture edge */
    SPI_CLOCKPHASE_2EDGE = (uint8_t)0x01 /*!< The second clock transition is the
                                            first data capture edge */
...
/**
 * @brief  SPI flags definition - Warning : FLAG value = mapping position
 * register
 */
typedef enum
{
 ...
 * @brief  Macros used by the assert_param function to check the different
 * functions parameters.
 * @{
 */

/**
 * @brief  Macro used by the assert_param function in order to check the data
 * direction mode values
 */
#define IS_SPI_DATA_DIRECTION_OK(MODE)                                         \
    (((MODE) == SPI_DATADIRECTION_2LINES_FULLDUPLEX) ||                        \
 ...
 * different sensitivity values for the CRC polynomial
 */
#define IS_SPI_CRC_POLYNOMIAL_OK(POLYNOMIAL) ((POLYNOMIAL) > (uint8_t)0x00)

/**
 * @brief  Macro used by the assert_param function in order to check the SPI
 * Mode values
 */
...
#define IS_SPI_POLARITY_OK(CLKPOL)                                             \
    (((CLKPOL) == SPI_CLOCKPOLARITY_LOW) ||                                    \
     ((CLKPOL) == SPI_CLOCKPOLARITY_HIGH))

/**
 * @brief  Macro used by the assert_param function in order to check the phase
 * values
 */
...
#define IS_SPI_FLAGS_OK(FLAG)                                                  \
    (((FLAG) == SPI_FLAG_OVR) || ((FLAG) == SPI_FLAG_MODF) ||                  \
...
#define IS_SPI_CONFIG_IT_OK(Interrupt)                                         \
    (((Interrupt) == SPI_IT_TXE) || ((Interrupt) == SPI_IT_RXNE) ||            \
     ((Interrupt) == SPI_IT_ERR) || ((Interrupt) == SPI_IT_WKUP))
...
#define IS_SPI_GET_IT_OK(ITPendingBit)                                         \
    (((ITPendingBit) == SPI_IT_OVR) || ((ITPendingBit) == SPI_IT_MODF) ||      \
     ((ITPendingBit) == SPI_IT_CRCERR) || ((ITPendingBit) == SPI_IT_WKUP) ||   \
     ((ITPendingBit) == SPI_IT_TXE) || ((ITPendingBit) == SPI_IT_RXNE))
