 ...
 * @brief   This file contains all functions prototype and macros for the EXTI
 *peripheral.
 ******************************************************************************
 * @attention
 *
...
typedef enum
{
    EXTI_SENSITIVITY_FALL_LOW =
      (uint8_t)0x00, /*!< Interrupt on Falling edge and Low level */
    EXTI_SENSITIVITY_RISE_ONLY =
      (uint8_t)0x01, /*!< Interrupt on Rising edge only */
    EXTI_SENSITIVITY_FALL_ONLY =
      (uint8_t)0x02, /*!< Interrupt on Falling edge only */
...
typedef enum
{
    EXTI_TLISENSITIVITY_FALL_ONLY =
      (uint8_t)0x00, /*!< Top Level Interrupt on Falling edge only */
    EXTI_TLISENSITIVITY_RISE_ONLY =
      (uint8_t)0x04 /*!< Top Level Interrupt on Rising edge only */
 ...
 * sensitivity values for PORTA to PORTE.
 */
...
#define IS_EXTI_PORT_OK(PORT)                                                  \
    (((PORT) == EXTI_PORT_GPIOA) || ((PORT) == EXTI_PORT_GPIOB) ||             \
     ((PORT) == EXTI_PORT_GPIOC) || ((PORT) == EXTI_PORT_GPIOD) ||             \
     ((PORT) == EXTI_PORT_GPIOE))

/**
 * @brief  Macro used by the assert function in order to check the different
 * values of the EXTI PinMask
 */
#define IS_EXTI_PINMASK_OK(PinMask)                                            \
    ((((PinMask) & (uint8_t)0x00) == (uint8_t)0x00) &&                         \
     ((PinMask) != (uint8_t)0x00))
