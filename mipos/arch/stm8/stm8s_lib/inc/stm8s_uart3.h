 ...
 * @brief   This file contains all functions prototypes and macros for the UART3
 *peripheral.
 ******************************************************************************
 * @attention
 *
...
typedef enum
{
    UART3_WAKEUP_IDLELINE = (uint8_t)0x00,   /**< 0x01 Idle Line wake up*/
...
typedef enum
{
    UART3_LINBREAKDETECTIONLENGTH_10BITS =
      (uint8_t)0x00, /**< 10 bits Lin Break detection */
    UART3_LINBREAKDETECTIONLENGTH_11BITS =
      (uint8_t)0x01 /**< 11 bits Lin Break detection */
...
typedef enum
{
    ...
    UART3_MODE_TXRX_ENABLE =
      (uint8_t)0x0C /**< 0x0C Receive Enable and Transmit enable*/
} UART3_Mode_TypeDef;
...
typedef enum
{
    UART3_LIN_MODE_MASTER = (uint8_t)0x00, /**<  LIN Master Mode*/
 ...
 * register. UART3_IT_TXE UART3_IT_TC UART3_IT_RXNE UART3_IT_IDLE UART3_IT_OR
 *   - For the UART3_IT_PE value, X means the flag position in the CR1 register.
 *   - For the UART3_IT_LBDF value, X means the flag position in the CR4
 * register.
 *   - For the UART3_IT_LHDF value, X means the flag position in the CR6
 * register. Y: Flag position
 *  - For the following values, Y means the flag (pending bit) position in the
 * SR register. UART3_IT_TXE UART3_IT_TC UART3_IT_RXNE UART3_IT_IDLE UART3_IT_OR
 *     UART3_IT_PE
 *  - For the UART3_IT_LBDF value, Y means the flag position in the CR4
 * register.
 *  - For the UART3_IT_LHDF value, Y means the flag position in the CR6
...
#define IS_UART3_FLAG_OK(Flag)                                                 \
    (((Flag) == UART3_FLAG_TXE) || ((Flag) == UART3_FLAG_TC) ||                \
     ((Flag) == UART3_FLAG_RXNE) || ((Flag) == UART3_FLAG_IDLE) ||             \
     ((Flag) == UART3_FLAG_OR_LHE) || ((Flag) == UART3_FLAG_NF) ||             \
     ((Flag) == UART3_FLAG_FE) || ((Flag) == UART3_FLAG_PE) ||                 \
     ((Flag) == UART3_FLAG_SBK) || ((Flag) == UART3_FLAG_LSF) ||               \
     ((Flag) == UART3_FLAG_LHDF) || ((Flag) == UART3_FLAG_LBDF))
...
#define IS_UART3_CLEAR_FLAG_OK(Flag)                                           \
    (((Flag) == UART3_FLAG_RXNE) || ((Flag) == UART3_FLAG_LHDF) ||             \
     ((Flag) == UART3_FLAG_LSF) || ((Flag) == UART3_FLAG_LBDF))
...
#define IS_UART3_CONFIG_IT_OK(Interrupt)                                       \
    (((Interrupt) == UART3_IT_PE) || ((Interrupt) == UART3_IT_TXE) ||          \
     ((Interrupt) == UART3_IT_TC) || ((Interrupt) == UART3_IT_RXNE_OR) ||      \
     ((Interrupt) == UART3_IT_IDLE) || ((Interrupt) == UART3_IT_LHDF) ||       \
...
#define IS_UART3_GET_IT_OK(ITPendingBit)                                       \
    (((ITPendingBit) == UART3_IT_TXE) || ((ITPendingBit) == UART3_IT_TC) ||    \
     ((ITPendingBit) == UART3_IT_RXNE) || ((ITPendingBit) == UART3_IT_IDLE) || \
     ((ITPendingBit) == UART3_IT_OR) || ((ITPendingBit) == UART3_IT_LBDF) ||   \
     ((ITPendingBit) == UART3_IT_LHDF) || ((ITPendingBit) == UART3_IT_PE))
...
#define IS_UART3_CLEAR_IT_OK(ITPendingBit)                                     \
    (((ITPendingBit) == UART3_IT_RXNE) || ((ITPendingBit) == UART3_IT_LHDF) || \
     ((ITPendingBit) == UART3_IT_LBDF))

/**
 * @brief Macro used by the assert_param function in order to check the
 * different sensitivity values for the MODEs
 */
#define IS_UART3_MODE_OK(Mode)                                                 \
    (((Mode) == (uint8_t)UART3_MODE_RX_ENABLE) ||                              \
     ...
     ((Mode) == (uint8_t)((uint8_t)UART3_MODE_TX_ENABLE |                      \
                          (uint8_t)UART3_MODE_RX_ENABLE)) ||                   \
     ((Mode) == (uint8_t)((uint8_t)UART3_MODE_TX_ENABLE |                      \
                          (uint8_t)UART3_MODE_RX_DISABLE)) ||                  \
     ((Mode) == (uint8_t)((uint8_t)UART3_MODE_TX_DISABLE |                     \
                          (uint8_t)UART3_MODE_RX_DISABLE)) ||                  \
     ((Mode) == (uint8_t)((uint8_t)UART3_MODE_TX_DISABLE |                     \
                          (uint8_t)UART3_MODE_RX_ENABLE)))
...
/**
 * @brief  Macro used by the assert_param function in order to check the
 * different sensitivity values for the UART3_StopBits
 */
...
#define IS_UART3_PARITY_OK(Parity)                                             \
    (((Parity) == UART3_PARITY_NO) || ((Parity) == UART3_PARITY_EVEN) ||       \
...
void UART3_LINBreakDetectionConfig(
  UART3_LINBreakDetectionLength_TypeDef UART3_LINBreakDetectionLength);
