...
typedef enum
{
    I2C_ADDMODE_7BIT = (uint8_t)0x00, /*!< 7-bit slave address (10-bit address
                                         not acknowledged) */
    I2C_ADDMODE_10BIT = (uint8_t)0x80 /*!< 10-bit slave address (7-bit address
                                         not acknowledged) */
} I2C_AddMode_TypeDef;
...
typedef enum
{
    /* SR1 register flags */
    I2C_FLAG_TXEMPTY =
      (uint16_t)0x0180, /*!< Transmit Data Register Empty flag */
    I2C_FLAG_RXNOTEMPTY =
      (uint16_t)0x0140, /*!< Read Data Register Not Empty flag */
    ...
    I2C_FLAG_TRANSFERFINISHED =
      (uint16_t)0x0104, /*!< Data Byte Transfer Finished flag */
    I2C_FLAG_ADDRESSSENTMATCHED =
      (uint16_t)0x0102, /*!< Address Sent/Matched (master/slave) flag */
    ...
    I2C_FLAG_ACKNOWLEDGEFAILURE =
      (uint16_t)0x0204, /*!< Acknowledge Failure Flag */
    I2C_FLAG_ARBITRATIONLOSS = (uint16_t)0x0202, /*!< Arbitration Loss Flag */
    I2C_FLAG_BUSERROR =
      (uint16_t)0x0201, /*!< Misplaced Start or Stop condition */

    /* SR3 register flags */
    I2C_FLAG_GENERALCALL =
      (uint16_t)0x0310, /*!< General Call header received Flag */
    I2C_FLAG_TRANSMITTERRECEIVER =
      (uint16_t)0x0304,                     /*!< Transmitter Receiver Flag */
...
typedef enum
{
    ...
    I2C_ITPENDINGBIT_TXEMPTY =
      (uint16_t)0x1680, /*!< Transmit Data Register Empty  */
    I2C_ITPENDINGBIT_RXNOTEMPTY =
      (uint16_t)0x1640, /*!< Read Data Register Not Empty  */
    ...
    I2C_ITPENDINGBIT_TRANSFERFINISHED =
      (uint16_t)0x1204, /*!< Data Byte Transfer Finished  */
    I2C_ITPENDINGBIT_ADDRESSSENTMATCHED =
      (uint16_t)0x1202, /*!< Address Sent/Matched (master/slave)  */
    I2C_ITPENDINGBIT_STARTDETECTION = (uint16_t)0x1201, /*!< Start bit sent  */

    /* SR2 register flags */
    I2C_ITPENDINGBIT_WAKEUPFROMHALT =
      (uint16_t)0x2220, /*!< Wake Up From Halt  */
    I2C_ITPENDINGBIT_OVERRUNUNDERRUN =
      (uint16_t)0x2108, /*!< Overrun/Underrun  */
    I2C_ITPENDINGBIT_ACKNOWLEDGEFAILURE =
      (uint16_t)0x2104, /*!< Acknowledge Failure  */
    I2C_ITPENDINGBIT_ARBITRATIONLOSS =
      (uint16_t)0x2102, /*!< Arbitration Loss  */
    I2C_ITPENDINGBIT_BUSERROR =
      (uint16_t)0x2101 /*!< Misplaced Start or Stop condition */
...
typedef enum
{
     ...
     * application. Using EV8_2 leads to a slower communication but ensures more
     ...
     * generates an ACK condition on the bus (if the acknowledge feature is
     * enabled through function I2C_AcknowledgeConfig()) and the events listed
     * above are set :
     *
     * 1) In normal case (only one address managed by the slave), when the
     * address sent by the master matches the own address of the peripheral
     * (configured by I2C_OwnAddress1 field) the
     * I2C_EVENT_SLAVE_XXX_ADDRESS_MATCHED event is set (where XXX could be
     ...
     * communication: master sends a stop condition and data transmission is
     * stopped.
     *
     * - Slave Transmitter mode:
     *    - EV3: When a byte has been transmitted by the slave and the
     * application is expecting the end of the byte transmission. The two events
     ...
     * software doesn't guarantee the EV3 is managed before the current byte end
     * of transfer.
     ...
     * slave has to stop sending data bytes and expect a Stop condition on the
     * bus.
     *
     *  @note In case the  user software does not guarantee that the event EV2
     * is managed before the current byte end of transfer, then user may check
     * on EV2 and BTF flag at the same time (ie. (I2C_EVENT_SLAVE_BYTE_RECEIVED
     * | I2C_FLAG_BTF)). In this case the communication may be slower.
     *
     */
    /* Slave RECEIVER mode --------------------------*/
    ...
    I2C_EVENT_SLAVE_BYTE_RECEIVED =
      (uint16_t)0x0240, /*!< BUSY and RXNE flags */
    /* --EV4  */
...
/**
 * @brief   Macro used by the assert function to check the different I2C duty
 * cycles.
 */
#define IS_I2C_DUTYCYCLE_OK(DUTY)                                              \
    (((DUTY) == I2C_DUTYCYCLE_2) || ((DUTY) == I2C_DUTYCYCLE_16_9))

/**
 * @brief   Macro used by the assert function to check the different
 * acknowledgement configuration
 */
#define IS_I2C_ACK_OK(ACK)                                                     \
    (((ACK) == I2C_ACK_NONE) || ((ACK) == I2C_ACK_CURR) ||                     \
     ((ACK) == I2C_ACK_NEXT))

/**
 * @brief   Macro used by the assert function to check the different I2C
 * addressing modes.
 */
#define IS_I2C_ADDMODE_OK(ADDMODE)                                             \
    (((ADDMODE) == I2C_ADDMODE_7BIT) || ((ADDMODE) == I2C_ADDMODE_10BIT))
...
#define IS_I2C_INTERRUPT_OK(IT)                                                \
    (((IT) == I2C_IT_ERR) || ((IT) == I2C_IT_EVT) || ((IT) == I2C_IT_BUF) ||   \
...
/**
 * @brief   Macro used by the assert function to check the different I2C
 * communcation direction.
 */
#define IS_I2C_DIRECTION_OK(DIR)                                               \
    (((DIR) == I2C_DIRECTION_TX) || ((DIR) == I2C_DIRECTION_RX))

/**
 * @brief   Macro used by the assert function to check the different I2C flags.
 */
#define IS_I2C_FLAG_OK(FLAG)                                                   \
    (((FLAG) == I2C_FLAG_TXEMPTY) || ((FLAG) == I2C_FLAG_RXNOTEMPTY) ||        \
     ((FLAG) == I2C_FLAG_STOPDETECTION) || ((FLAG) == I2C_FLAG_HEADERSENT) ||  \
     ...
     ((FLAG) == I2C_FLAG_ARBITRATIONLOSS) || ((FLAG) == I2C_FLAG_BUSERROR) ||  \
     ((FLAG) == I2C_FLAG_GENERALCALL) ||                                       \
     ((FLAG) == I2C_FLAG_TRANSMITTERRECEIVER) ||                               \
     ((FLAG) == I2C_FLAG_BUSBUSY) || ((FLAG) == I2C_FLAG_MASTERSLAVE))
/**
...
#define IS_I2C_EVENT_OK(EVENT)                                                 \
    (((EVENT) == I2C_EVENT_SLAVE_TRANSMITTER_ADDRESS_MATCHED) ||               \
     ...
     ((EVENT) ==                                                               \
      (I2C_EVENT_SLAVE_BYTE_RECEIVED | (uint16_t)I2C_FLAG_GENERALCALL)) ||     \
     ((EVENT) == I2C_EVENT_SLAVE_BYTE_TRANSMITTED) ||                          \
     ((EVENT) ==                                                               \
      (I2C_EVENT_SLAVE_BYTE_TRANSMITTED | (uint16_t)I2C_FLAG_GENERALCALL)) ||  \
...
/**
 * @brief   Macro used by the assert function to check that I2C Input clock
 * frequency must be between 1MHz and 50MHz.
 */
...
/**
 * @brief   Macro used by the assert function to check that I2C Output clock
 * frequency must be between 1Hz and 400kHz.
 */
 ...
 *event (can be the combination of one or more flags). It returns SUCCESS if the
 *current status includes the given flags and returns ERROR if one or more flags
 *are missing in the current status.
 *    - When to use:
 ...
 *communication hold or corrupted real state. In this case, it is advised to use
 *error interrupts to monitor the error events and handle them in the interrupt
 *IRQ handler.
 *
 *        @note
 ...
 *           I2Cx_IRQHandler() function in order to determine which error
 *occurred.
 ...
 *     & SR3 status registers in a single word (uint16_t) (Status Register 3
 *value is shifted left by 8 bits and concatenated to Status Register 1).
 *     - When to use:
 ...
 *user to choose when an event is accepted (when all events flags are set and no
 ...
 *flags are cleared when the status register is accessed. So checking the status
