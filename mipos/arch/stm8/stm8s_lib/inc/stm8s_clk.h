 ...
 * @brief   This file contains all functions prototype and macros for the CLK
 *peripheral.
 ******************************************************************************
 * @attention
 *
...
typedef enum
{
    CLK_SWITCHMODE_MANUAL =
      (uint8_t)0x00, /*!< Enable the manual clock switching mode */
    CLK_SWITCHMODE_AUTO =
      (uint8_t)0x01 /*!< Enable the automatic clock switching mode */
...
typedef enum
{
    CLK_CSSCONFIG_ENABLEWITHIT =
      (uint8_t)0x05, /*!< Enable CSS with detection interrupt */
    CLK_CSSCONFIG_ENABLE =
      (uint8_t)0x01, /*!< Enable CSS without detection interrupt */
    CLK_CSSCONFIG_DISABLE = (uint8_t)0x00 /*!< Leave CSS desactivated (to be
                                             used in CLK_Init() function) */
...
typedef enum
{
  ...
  defined(STM8AF52Ax) || defined(STM8AF62Ax)
    CLK_PERIPHERAL_UART1 =
      (uint8_t)0x02, /*!< Peripheral Clock Enable 1, UART1 */
#else
    CLK_PERIPHERAL_UART1 =
      (uint8_t)0x03, /*!< Peripheral Clock Enable 1, UART1 */
#endif
    CLK_PERIPHERAL_UART2 =
      (uint8_t)0x03, /*!< Peripheral Clock Enable 1, UART2 */
    CLK_PERIPHERAL_UART3 =
      (uint8_t)0x03, /*!< Peripheral Clock Enable 1, UART3 */
    CLK_PERIPHERAL_TIMER6 =
      (uint8_t)0x04, /*!< Peripheral Clock Enable 1, Timer6 */
    CLK_PERIPHERAL_TIMER4 =
      (uint8_t)0x04, /*!< Peripheral Clock Enable 1, Timer4 */
    CLK_PERIPHERAL_TIMER5 =
      (uint8_t)0x05, /*!< Peripheral Clock Enable 1, Timer5 */
    CLK_PERIPHERAL_TIMER2 =
      (uint8_t)0x05, /*!< Peripheral Clock Enable 1, Timer2 */
    CLK_PERIPHERAL_TIMER3 =
      (uint8_t)0x06, /*!< Peripheral Clock Enable 1, Timer3 */
    CLK_PERIPHERAL_TIMER1 =
      (uint8_t)0x07, /*!< Peripheral Clock Enable 1, Timer1 */
...
{
    CLK_FLAG_LSIRDY =
      (uint16_t)0x0110, /*!< Low speed internal oscillator ready Flag */
    CLK_FLAG_HSIRDY =
      (uint16_t)0x0102, /*!< High speed internal oscillator ready Flag */
    CLK_FLAG_HSERDY =
      (uint16_t)0x0202, /*!< High speed external oscillator ready Flag */
    CLK_FLAG_SWIF = (uint16_t)0x0308,  /*!< Clock switch interrupt Flag */
    CLK_FLAG_SWBSY = (uint16_t)0x0301, /*!< Switch busy Flag */
    CLK_FLAG_CSSD =
      (uint16_t)0x0408, /*!< Clock security system detection Flag */
    CLK_FLAG_AUX =
      (uint16_t)0x0402, /*!< Auxiliary oscillator connected to master clock */
...
typedef enum
{
    CLK_PRESCALER_HSIDIV1 =
      (uint8_t)0x00, /*!< High speed internal clock prescaler: 1 */
    CLK_PRESCALER_HSIDIV2 =
      (uint8_t)0x08, /*!< High speed internal clock prescaler: 2 */
    CLK_PRESCALER_HSIDIV4 =
      (uint8_t)0x10, /*!< High speed internal clock prescaler: 4 */
    CLK_PRESCALER_HSIDIV8 =
      (uint8_t)0x18, /*!< High speed internal clock prescaler: 8 */
    ...
    CLK_PRESCALER_CPUDIV16 =
      (uint8_t)0x84, /*!< CPU clock division factors 16 */
    CLK_PRESCALER_CPUDIV32 =
      (uint8_t)0x85, /*!< CPU clock division factors 32 */
    CLK_PRESCALER_CPUDIV64 =
      (uint8_t)0x86, /*!< CPU clock division factors 64 */
    CLK_PRESCALER_CPUDIV128 =
      (uint8_t)0x87 /*!< CPU clock division factors 128 */
 ...
 */
#define CLK_TIMEOUT                                                            \
    ((uint16_t)0xFFFF) /*!< Max Timeout for the clock switch operation. */
/**
 * @}
 */
...
/**
 * @brief  Macros used by the assert function in order to check the clock
 * switching modes.
 */
#define IS_CLK_SWITCHMODE_OK(MODE)                                             \
    (((MODE) == CLK_SWITCHMODE_MANUAL) || ((MODE) == CLK_SWITCHMODE_AUTO))

/**
 * @brief  Macros used by the assert function in order to check the current
 * clock state.
 */
#define IS_CLK_CURRENTCLOCKSTATE_OK(STATE)                                     \
    (((STATE) == CLK_CURRENTCLOCKSTATE_DISABLE) ||                             \
     ((STATE) == CLK_CURRENTCLOCKSTATE_ENABLE))

/**
 * @brief  Macros used by the assert function in order to check the CSS
 * configuration.
 */
...
#define IS_CLK_SOURCE_OK(SOURCE)                                               \
    (((SOURCE) == CLK_SOURCE_HSI) || ((SOURCE) == CLK_SOURCE_LSI) ||           \
...
#define IS_CLK_OUTPUT_OK(OUTPUT)                                               \
    (((OUTPUT) == CLK_OUTPUT_HSI) || ((OUTPUT) == CLK_OUTPUT_HSE) ||           \
     ((OUTPUT) == CLK_OUTPUT_LSI) || ((OUTPUT) == CLK_OUTPUT_CPU) ||           \
     ((OUTPUT) == CLK_OUTPUT_CPUDIV2) || ((OUTPUT) == CLK_OUTPUT_CPUDIV4) ||   \
     ((OUTPUT) == CLK_OUTPUT_CPUDIV8) || ((OUTPUT) == CLK_OUTPUT_CPUDIV16) ||  \
     ((OUTPUT) == CLK_OUTPUT_CPUDIV32) || ((OUTPUT) == CLK_OUTPUT_CPUDIV64) || \
     ((OUTPUT) == CLK_OUTPUT_HSIRC) || ((OUTPUT) == CLK_OUTPUT_MASTER) ||      \
...
#define IS_CLK_FLAG_OK(FLAG)                                                   \
    (((FLAG) == CLK_FLAG_LSIRDY) || ((FLAG) == CLK_FLAG_HSIRDY) ||             \
     ((FLAG) == CLK_FLAG_HSERDY) || ((FLAG) == CLK_FLAG_SWIF) ||               \
     ((FLAG) == CLK_FLAG_SWBSY) || ((FLAG) == CLK_FLAG_CSSD) ||                \
     ((FLAG) == CLK_FLAG_AUX) || ((FLAG) == CLK_FLAG_CCOBSY) ||                \
     ((FLAG) == CLK_FLAG_CCORDY))

/**
 * @brief  Macros used by the assert function in order to check the different
 * clock IT pending bits.
 */
#define IS_CLK_IT_OK(IT) (((IT) == CLK_IT_CSSD) || ((IT) == CLK_IT_SWIF))

/**
 * @brief  Macros used by the assert function in order to check the different
 * HSI prescaler values.
 */
...
/**
 * @brief  Macros used by the assert function in order to check the different
 * SWIM dividers values.
 */
#define IS_CLK_SWIMDIVIDER_OK(SWIMDIVIDER)                                     \
    (((SWIMDIVIDER) == CLK_SWIMDIVIDER_2) ||                                   \
     ((SWIMDIVIDER) == CLK_SWIMDIVIDER_OTHER))
...
void CLK_AdjustHSICalibrationValue(
