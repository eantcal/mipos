 ...
 * @brief   This file contains all functions prototypes and macros for the UART1
 *peripheral.
 ******************************************************************************
 * @attention
 *
 ...
 * register. UART1_IT_TXE UART1_IT_TC UART1_IT_RXNE UART1_IT_IDLE UART1_IT_OR
 *   - For the UART1_IT_PE value, X means the flag position in the CR1 register.
 *   - For the UART1_IT_LBDF value, X means the flag position in the CR4
 * register. Y: Flag position
 *  - For the following values, Y means the flag (pending bit) position in the
 * SR register. UART1_IT_TXE UART1_IT_TC UART1_IT_RXNE UART1_IT_IDLE UART1_IT_OR
 *     UART1_IT_PE
 *  - For the UART1_IT_LBDF value, Y means the flag position in the CR4
...
#define IS_UART1_MODE_OK(Mode)                                                 \
    (((Mode) == (uint8_t)UART1_MODE_RX_ENABLE) ||                              \
     ...
     ((Mode) == (uint8_t)((uint8_t)UART1_MODE_TX_ENABLE |                      \
                          (uint8_t)UART1_MODE_RX_ENABLE)) ||                   \
     ((Mode) == (uint8_t)((uint8_t)UART1_MODE_TX_ENABLE |                      \
                          (uint8_t)UART1_MODE_RX_DISABLE)) ||                  \
     ((Mode) == (uint8_t)((uint8_t)UART1_MODE_TX_DISABLE |                     \
                          (uint8_t)UART1_MODE_RX_DISABLE)) ||                  \
     ((Mode) == (uint8_t)((uint8_t)UART1_MODE_TX_DISABLE |                     \
                          (uint8_t)UART1_MODE_RX_ENABLE)))
...
#define IS_UART1_SYNCMODE_OK(SyncMode)                                         \
    (!((((SyncMode) & (((uint8_t)UART1_SYNCMODE_CLOCK_ENABLE) |                \
                       ((uint8_t)UART1_SYNCMODE_CLOCK_DISABLE))) ==            \
        (((uint8_t)UART1_SYNCMODE_CLOCK_ENABLE) |                              \
         ((uint8_t)UART1_SYNCMODE_CLOCK_DISABLE))) ||                          \
       (((SyncMode) & (((uint8_t)UART1_SYNCMODE_CPOL_LOW) |                    \
                       ((uint8_t)UART1_SYNCMODE_CPOL_HIGH))) ==                \
        (((uint8_t)UART1_SYNCMODE_CPOL_LOW) |                                  \
         ((uint8_t)UART1_SYNCMODE_CPOL_HIGH))) ||                              \
       (((SyncMode) & (((uint8_t)UART1_SYNCMODE_CPHA_MIDDLE) |                 \
                       ((uint8_t)UART1_SYNCMODE_CPHA_BEGINING))) ==            \
        (((uint8_t)UART1_SYNCMODE_CPHA_MIDDLE) |                               \
         ((uint8_t)UART1_SYNCMODE_CPHA_BEGINING))) ||                          \
       (((SyncMode) & (((uint8_t)UART1_SYNCMODE_LASTBIT_DISABLE) |             \
                       ((uint8_t)UART1_SYNCMODE_LASTBIT_ENABLE))) ==           \
        (((uint8_t)UART1_SYNCMODE_LASTBIT_DISABLE) |                           \
         ((uint8_t)UART1_SYNCMODE_LASTBIT_ENABLE)))))
...
#define IS_UART1_FLAG_OK(Flag)                                                 \
    (((Flag) == UART1_FLAG_TXE) || ((Flag) == UART1_FLAG_TC) ||                \
     ((Flag) == UART1_FLAG_RXNE) || ((Flag) == UART1_FLAG_IDLE) ||             \
     ((Flag) == UART1_FLAG_OR) || ((Flag) == UART1_FLAG_NF) ||                 \
     ((Flag) == UART1_FLAG_FE) || ((Flag) == UART1_FLAG_PE) ||                 \
     ((Flag) == UART1_FLAG_SBK) || ((Flag) == UART1_FLAG_LBDF))
/**
 * @brief  Macro used by the assert_param function in order to check the
 * different sensitivity values for the FLAGs that can be cleared by writing 0
 */
#define IS_UART1_CLEAR_FLAG_OK(Flag)                                           \
    (((Flag) == UART1_FLAG_RXNE) || ((Flag) == UART1_FLAG_LBDF))
...
#define IS_UART1_CONFIG_IT_OK(Interrupt)                                       \
    (((Interrupt) == UART1_IT_PE) || ((Interrupt) == UART1_IT_TXE) ||          \
     ((Interrupt) == UART1_IT_TC) || ((Interrupt) == UART1_IT_RXNE_OR) ||      \
     ((Interrupt) == UART1_IT_IDLE) || ((Interrupt) == UART1_IT_LBDF))
...
#define IS_UART1_GET_IT_OK(ITPendingBit)                                       \
    (((ITPendingBit) == UART1_IT_TXE) || ((ITPendingBit) == UART1_IT_TC) ||    \
     ((ITPendingBit) == UART1_IT_RXNE) || ((ITPendingBit) == UART1_IT_IDLE) || \
     ((ITPendingBit) == UART1_IT_OR) || ((ITPendingBit) == UART1_IT_LBDF) ||   \
...
#define IS_UART1_CLEAR_IT_OK(ITPendingBit)                                     \
    (((ITPendingBit) == UART1_IT_RXNE) || ((ITPendingBit) == UART1_IT_LBDF))
...
/**
 * @brief  Macro used by the assert_param function in order to check the
 * different sensitivity values for the UART1_StopBits
 */
#define IS_UART1_STOPBITS_OK(StopBit)                                          \
    (((StopBit) == UART1_STOPBITS_1) || ((StopBit) == UART1_STOPBITS_0_5) ||   \
     ((StopBit) == UART1_STOPBITS_2) || ((StopBit) == UART1_STOPBITS_1_5))

/**
 * @brief Macro used by the assert_param function in order to check the
 * different sensitivity values for the Parity
 */
#define IS_UART1_PARITY_OK(Parity)                                             \
    (((Parity) == UART1_PARITY_NO) || ((Parity) == UART1_PARITY_EVEN) ||       \
...
void UART1_LINBreakDetectionConfig(
  UART1_LINBreakDetectionLength_TypeDef UART1_LINBreakDetectionLength);
