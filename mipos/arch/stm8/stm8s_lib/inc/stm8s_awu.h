 ...
 * @brief   This file contains all functions prototype and macros for the AWU
 *peripheral.
 ******************************************************************************
 * @attention
 *
...
#define LSI_FREQUENCY_MIN                                                      \
    ((uint32_t)110000) /*!< LSI minimum value in Hertz                         \
                        */
#define LSI_FREQUENCY_MAX                                                      \
    ((uint32_t)150000) /*!< LSI maximum value in Hertz                         \
                        */
...
#define IS_AWU_TIMEBASE_OK(TB)                                                 \
    (((TB) == AWU_TIMEBASE_NO_IT) || ((TB) == AWU_TIMEBASE_250US) ||           \
     ((TB) == AWU_TIMEBASE_500US) || ((TB) == AWU_TIMEBASE_1MS) ||             \
     ((TB) == AWU_TIMEBASE_2MS) || ((TB) == AWU_TIMEBASE_4MS) ||               \
     ((TB) == AWU_TIMEBASE_8MS) || ((TB) == AWU_TIMEBASE_16MS) ||              \
     ((TB) == AWU_TIMEBASE_32MS) || ((TB) == AWU_TIMEBASE_64MS) ||             \
     ((TB) == AWU_TIMEBASE_128MS) || ((TB) == AWU_TIMEBASE_256MS) ||           \
     ((TB) == AWU_TIMEBASE_512MS) || ((TB) == AWU_TIMEBASE_1S) ||              \
     ((TB) == AWU_TIMEBASE_2S) || ((TB) == AWU_TIMEBASE_12S) ||                \
     ((TB) == AWU_TIMEBASE_30S))

/**
 * @brief    Macro used by the assert function to check the LSI frequency (in
 * Hz)
 */
