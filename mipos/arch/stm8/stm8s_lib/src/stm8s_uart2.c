...
void UART2_DeInit(void)
{
    ...
    UART2->BRR2 =
      UART2_BRR2_RESET_VALUE; /*  Set UART2_BRR2 to reset value 0x00 */
    UART2->BRR1 =
      UART2_BRR1_RESET_VALUE; /*  Set UART2_BRR1 to reset value 0x00 */
                ...
                UART2_Mode_TypeDef Mode)
{
    ...
    UART2->CR3 |=
      (uint8_t)((uint8_t)SyncMode &
                (uint8_t)(UART2_CR3_CPOL | UART2_CR3_CPHA | UART2_CR3_LBCL));
 ...
 * @param  UART2_IT specifies the UART2 interrupt sources to be enabled or
 ...
 *         - UART2_IT_RXNE_OR: Receive Data register not empty/Over run error
 * interrupt
...
void UART2_ITConfig(UART2_IT_TypeDef UART2_IT, FunctionalState NewState)
{
    ...
    itpos =
      (uint8_t)((uint8_t)1 << (uint8_t)((uint8_t)UART2_IT & (uint8_t)0x0F));
 ...
 * @param  UART2_LINBreakDetectionLength specifies the LIN break detection
  ...
  UART2_LINBreakDetectionLength_TypeDef UART2_LINBreakDetectionLength)
{
    /* Check parameters */
    assert_param(
      IS_UART2_LINBREAKDETECTIONLENGTH_OK(UART2_LINBreakDetectionLength));
 ...
 *         This parameter can be any of the @ref UART2_LinAutosync_TypeDef
 * values.
...
void UART2_ReceiverWakeUpCmd(FunctionalState NewState)
{
    ...
    if (NewState != DISABLE) {
        ...
        /* Disable the mute mode UART2 by clearing the RWU bit in the CR1
         * register */
...
uint16_t UART2_ReceiveData9(void)
{
    ...
    temp =
      ((uint16_t)(((uint16_t)((uint16_t)UART2->CR1 & (uint16_t)UART2_CR1_R8))
                  << 1));
 ...
 *           operation to UART2_DR register (UART2_SendData8() or
 * UART2_SendData9()).
 *
 ...
 *           (UART2_GetITStatus()) followed by a read operation to UART2_DR
 * register (UART2_ReceiveData8() or UART2_ReceiveData9()).
 *
