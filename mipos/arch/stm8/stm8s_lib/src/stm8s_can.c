 ...
 * @param   CAN_MasterCtrl : Master control option, can be one or a combination
 * of @ref CAN_MasterCtrl_TypeDef.
 * @param   CAN_Mode : CAN mode , can be one of @ref CAN_Mode_TypeDef.
 * @param   CAN_SynJumpWidth : CAN Synchronisation Jump Width , can be one of
 * @ref CAN_SynJumpWidth_TypeDef.
 * @param   CAN_BitSeg1 : CAN bit segment 1 , can be one of @ref
 * CAN_BitSeg1_TypeDef.
 * @param   CAN_BitSeg2 : CAN bit segment 2 , can be one of @ref
 * CAN_BitSeg2_TypeDef.
 * @param   CAN_Prescaler : CAN Baud Rate Prescaler , can be a value from 0x01
 * to 0xFF.
 * @retval Indicates if initialization is succeed. it can be one of @ref
 * CAN_InitStatus_TypeDef enumeration.
 */
                                ...
                                uint8_t CAN_Prescaler)
{
    ...
    while (((uint8_t)(CAN->MSR & CAN_MSR_INAK) != 0x01) &&
           ((uint16_t)timeout != 0)) {
        timeout--;
    }
    ...
    if ((CAN->MSR & CAN_MSR_INAK) != CAN_MSR_INAK) {
        ...
        /* Set the time triggered communication mode  &  Set the automatic
        bus-off management  &   Set the automatic wake-up mode &     Set the no
        automatic retransmission & Set the receive FIFO locked mode & Set the
        ...
        CAN->PSR = CAN_Page_Config;
        CAN->Page.Config.BTR1 =
          (uint8_t)((uint8_t)(CAN_Prescaler - (uint8_t)1) | CAN_SynJumpWidth);
...
/**
 * @brief  Initializes the CAN peripheral Filter according to the specified
 * parameters.
 * @param   CAN_FilterNumber : CAN Filter number , can be one of @ref
 * CAN_FilterNumber_TypeDef
 * @param   CAN_FilterActivation : CAN Filter Activation state , can be one of
 * @ref FunctionalState
 * @param   CAN_FilterMode : CAN Filter Mode , can be one of @ref
 * CAN_FilterMode_TypeDef
 * @param   CAN_FilterScale : CAN Filter Scale , can be one of @ref
 ...
 * @param   CAN_FilterIDMask1 : CAN Filter ID 1/ Mask 1  , can be a value from
 * 0x00 to 0xFF depending of CAN_FilterMode parameter
 * @param   CAN_FilterIDMask2 : CAN Filter ID 2/ Mask 2 , can be a value from
 * 0x00 to 0xFF depending of CAN_FilterMode parameter
 * @param   CAN_FilterIDMask3 : CAN Filter ID 3/ Mask 3 , can be a value from
 * 0x00 to 0xFF depending of CAN_FilterMode parameter
 * @param   CAN_FilterIDMask4 : CAN Filter ID 4/ Mask 4 , can be a value from
 * 0x00 to 0xFF depending of CAN_FilterMode parameter
 * @retval None
 */
                    ...
                    uint8_t CAN_FilterIDMask4)
{
    ...
    if (can_page_filter == CAN_Page_Filter01) /* FCR1 */
    {
        ...
        CAN->Page.Config.FCR1 &= (uint8_t)(~(
          uint8_t)(fact | (uint8_t)((uint8_t)(CAN_FCR1_FSC00 | CAN_FCR1_FSC01)
                                    << fsc)));
...
void CAN_ITConfig(CAN_IT_TypeDef CAN_IT, FunctionalState NewState)
{
    ...
    tmperrorinterrupt =
      (uint8_t)((uint8_t)((uint16_t)tmperrorinterrupt & 0xF0) |
 ...
 * @param   CAN_ST7Compatibility : CAN ST7 Compatibility , this parameter can be
 * one of @ref CAN_ST7Compatibility_TypeDef enumeration.
 * @retval None
 */
 ...
 * @param   CAN_Id the ID number of the message, its size depends on @ref
 * CAN_IDE value.
 * @param[in] CAN_IDE the ID type of the message, this parameter can be one of
 * the @ref CAN_Id_TypeDef enumeration.
 * @param[in] CAN_RTR the message type, this parameter can be one of the @ref
 * CAN_RTR_TypeDef enumeration.
 * @param[in] CAN_DLC the number of data in the message type, this parameter can
 * be a value between 0 to 7.
 * @param[in] CAN_Data pointer to a the @ref uint8_t table which contains data
 * to sent.
 * @retval Transmit Status, this returned value can be one of the @ref
 * CAN_TxStatus_TypeDef enumeration.
 */
 ...
 * transmission, can be on of  @ref CAN_TransmitMailBox_TypeDef.
  ...
  CAN_TransmitMailBox_TypeDef CAN_TransmitMailbox)
{
    ...
    switch (CAN_TransmitMailbox) {
        case (CAN_TransmitMailBox_0):
            tmpstate =
              (uint8_t)((CAN->TSR & (uint8_t)(CAN_TSR_RQCP0 | CAN_TSR_TXOK0)));
            tmpstate |= (uint8_t)((CAN->TPR & CAN_TPR_TME0));
            break;
        case (CAN_TransmitMailBox_1):
            tmpstate =
              (uint8_t)((uint8_t)(CAN->TSR &
                                  (uint8_t)(CAN_TSR_RQCP1 | CAN_TSR_TXOK1)) >>
                        1);
        ...
        case (CAN_TransmitMailBox_2):
            tmpstate =
              (uint8_t)((uint8_t)(CAN->TSR &
                                  (uint8_t)(CAN_TSR_RQCP2 | CAN_TSR_TXOK2)) >>
                        2);
 ...
 * @param   TransmitMailbox : the Transmission mailbox, can be one of
 ...
 * In order to get these data, use CAN_GetReceivedId(), CAN_GetReceivedIDE(),
 * CAN_GetReceivedRTR(), CAN_GetReceivedDLC(), CAN_GetReceivedFMI() and
 * CAN_GetReceivedData() functions.
 * @param  None
 * @retval None
 */
void CAN_Receive(void)
{
    ...
    if (_IDE != CAN_Id_Standard) {
        ...
        _Id = (uint32_t)CAN_EXTID_SIZE &
              ((CAN->Page.RxFIFO.MIDR4) | temp1 | temp2 | temp3);
    } else {
        temp1 = (uint16_t)((
          uint16_t)((uint16_t)((uint16_t)CAN->Page.RxFIFO.MIDR1 & 0x1F) << 6));
        temp2 =
          (uint16_t)((uint16_t)((uint16_t)CAN->Page.RxFIFO.MIDR2 >> 2) & 0x3F);
...
CAN_Sleep_TypeDef CAN_Sleep(void)
{
    ...
    /* Request Sleep mode */
    CAN->MCR =
      (uint8_t)((uint8_t)(CAN->MCR & (uint8_t)(~CAN_MCR_INRQ)) | CAN_MCR_SLEEP);
  ...
  CAN_OperatingMode_TypeDef CAN_OperatingMode)
{
    ...
    if (CAN_OperatingMode == CAN_OperatingMode_Initialization) {
        /* Request initialisation */
        CAN->MCR = (uint8_t)((uint8_t)(CAN->MCR & (uint8_t)(~CAN_MCR_SLEEP)) |
                             CAN_MCR_INRQ);
        ...
        /* Request Sleep mode */
        CAN->MCR = (uint8_t)((uint8_t)(CAN->MCR & (uint8_t)(~CAN_MCR_INRQ)) |
                             CAN_MCR_SLEEP);
 ...
 * @param   CAN_FLAG : Flag to be cleared, can be one of the following
 * parameters: CAN_FLAG_RQCP0            Request MailBox0  Flag CAN_FLAG_RQCP1
 * Request MailBox1  Flag CAN_FLAG_RQCP2            Request MailBox2  Flag
 ...
 * @param   CAN_FLAG: specifies the flag to check, can be one of @ref
 * CAN_FLAG_TypeDef enumeration.
 ...
 * @param   CAN_IT: specifies the CAN interrupt source to check, can be one of
 * @ref CAN_IT_TypeDef.
    ...
    switch (CAN_IT) {
        ...
        case CAN_IT_ERR:
            ...
            if ((CAN->Page.Config.EIER & CAN_EIER_ERRIE) != RESET) {
                pendingbitstatus = CheckITStatus(CAN->Page.Config.ESR,
                                                 CAN_ESR_EWGF | CAN_ESR_EPVF |
                                                   CAN_ESR_BOFF | CAN_ESR_LEC);
            } else {
                pendingbitstatus = RESET;
            }
            break;
        ...
        case CAN_IT_EWG:
            ...
            if ((CAN->Page.Config.EIER & CAN_EIER_EWGIE) != RESET) {
                pendingbitstatus =
                  CheckITStatus(CAN->Page.Config.ESR, CAN_ESR_EWGF);
            } else {
                pendingbitstatus = RESET;
            }
            break;
        ...
        case CAN_IT_EPV:
            ...
            if ((CAN->Page.Config.EIER & CAN_EIER_EPVIE) != RESET) {
                pendingbitstatus =
                  CheckITStatus(CAN->Page.Config.ESR, CAN_ESR_EPVF);
            } else {
                pendingbitstatus = RESET;
            }
            break;
        case CAN_IT_BOF:
            ...
            if ((CAN->Page.Config.EIER & CAN_EIER_BOFIE) != RESET) {
                pendingbitstatus =
                  CheckITStatus(CAN->Page.Config.ESR, CAN_ESR_BOFF);
            } else {
                pendingbitstatus = RESET;
            }
            break;
        case CAN_IT_LEC:
            ...
            if ((CAN->Page.Config.EIER & CAN_EIER_LECIE) != RESET) {
                pendingbitstatus =
                  CheckITStatus(CAN->Page.Config.ESR, CAN_ESR_LEC);
            } else {
                pendingbitstatus = RESET;
            }
            break;
