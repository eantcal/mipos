...
/**
 * @brief  Enable or Disable the I2C acknowledge and position acknowledge
 * feature.
 ...
 * @param   Direction : Specifies whether the I2C device will be a Transmitter
 * or a Receiver. This parameter can be any of the @ref I2C_Direction_TypeDef
 * enumeration.
 * @retval None
 */
 ...
 *           I2C_IRQHandler() function in order to determine which error
 *occurred.
...
ErrorStatus I2C_CheckEvent(I2C_Event_TypeDef I2C_Event)
{
    ...
    if (I2C_Event == I2C_EVENT_SLAVE_ACK_FAILURE) {
        ...
        flag2 = I2C->SR3;
        lastevent =
          ((uint16_t)((uint16_t)flag2 << (uint16_t)8) | (uint16_t)flag1);
    }
 ...
 *                       - I2C_FLAG_OVERRUNUNDERRUN: Overrun/Underrun flag
 * (Slave mode)
 *                       - I2C_FLAG_ACKNOWLEDGEFAILURE: Acknowledge failure flag
 *                       - I2C_FLAG_ARBITRATIONLOSS: Arbitration lost flag
 * (Master mode)
 ...
 *               - I2C_ITPENDINGBIT_OVERRUNUNDERRUN: Overrun/Underrun flag
 * (Slave mode)
 *               - I2C_ITPENDINGBIT_ACKNOWLEDGEFAILURE: Acknowledge failure flag
 *               - I2C_ITPENDINGBIT_ARBITRATIONLOSS: Arbitration lost flag
 * (Master mode)
 *               - I2C_ITPENDINGBIT_BUSERROR: Bus error flag
 *               - I2C_ITPENDINGBIT_TXEMPTY: Data register empty flag
 * (Transmitter)
 *               - I2C_ITPENDINGBIT_RXNOTEMPTY: Data register not empty
 * (Receiver) flag
 *               - I2C_ITPENDINGBIT_STOPDETECTION: Stop detection flag (Slave
 * mode)
 *               - I2C_ITPENDINGBIT_HEADERSENT: 10-bit header sent flag (Master
 * mode)
 *               - I2C_ITPENDINGBIT_TRANSFERFINISHED: Byte transfer finished
 * flag
 *               - I2C_ITPENDINGBIT_ADDRESSSENTMATCHED: Address sent flag
 * (Master mode) “ADSL” Address matched flag (Slave mode)“ENDAD”
...
ITStatus I2C_GetITStatus(I2C_ITPendingBit_TypeDef I2C_ITPendingBit)
{
    ...
    tempregister =
      (uint8_t)(((uint16_t)((uint16_t)I2C_ITPendingBit & ITEN_Mask)) >> 8);

    /* Check if the interrupt source is enabled or not */
    enablestatus = (uint8_t)(I2C->ITR & (uint8_t)tempregister);

    if ((uint16_t)((uint16_t)I2C_ITPendingBit & REGISTER_Mask) ==
        REGISTER_SR1_Index) {
        /* Check the status of the specified I2C flag */
 ...
 *            This parameter can be any combination of the following values:
 *                 - I2C_ITPENDINGBIT_WAKEUPFROMHALT: Wakeup from Halt
 *                 - I2C_ITPENDINGBIT_OVERRUNUNDERRUN: Overrun/Underrun
 * interrupt (Slave mode)
 *                 - I2C_ITPENDINGBIT_ACKNOWLEDGEFAILURE: Acknowledge failure
 * interrupt
 *                 - I2C_ITPENDINGBIT_ARBITRATIONLOSS: Arbitration lost
 * interrupt (Master mode)
