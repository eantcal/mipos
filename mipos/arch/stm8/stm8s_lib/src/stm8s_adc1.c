               ...
               FunctionalState ADC1_SchmittTriggerState)
{
    ...
    /* Select the prescaler division factor according to ADC1_PrescalerSelection
     * values */
...
/**
 * @brief  Enables or Disables the ADC1 data store into the Data Buffer
 * registers rather than in the Data Register
 * @param   NewState: specifies the selected mode enabled or disabled state.
 * @retval None
 */
  ...
  FunctionalState NewState)
{
    ...
    if (ADC1_SchmittTriggerChannel == ADC1_SCHMITTTRIG_ALL) {
        ...
        if (NewState != DISABLE) {
            ADC1->TDRL &= (uint8_t)(~(
              uint8_t)((uint8_t)0x01 << (uint8_t)ADC1_SchmittTriggerChannel));
        } else /* NewState == DISABLE */
        {
            ADC1->TDRL |=
              (uint8_t)((uint8_t)0x01 << (uint8_t)ADC1_SchmittTriggerChannel);
...
uint16_t ADC1_GetConversionValue(void)
{
    ...
    } else /* Left alignment */
    {
        ...
        temph = (uint16_t)((uint16_t)((uint16_t)templ << 6) |
                           (uint16_t)((uint16_t)temph << 8));
    }
                           ...
                           FunctionalState NewState)
{
    ...
    if (Channel < (uint8_t)8) {
        ...
        {
            ADC1->AWCRH &= (uint8_t) ~(
              uint8_t)((uint8_t)1 << (uint8_t)(Channel - (uint8_t)8));
        }
    }
}
...
uint16_t ADC1_GetBufferValue(uint8_t Buffer)
{
    ...
    {
        /* Read LSB first */
        templ = *(uint8_t*)(uint16_t)((uint16_t)ADC1_BaseAddress +
                                      (uint8_t)(Buffer << 1) + 1);
        /* Then read MSB */
        temph = *(uint8_t*)(uint16_t)((uint16_t)ADC1_BaseAddress +
                                      (uint8_t)(Buffer << 1));
    ...
    } else /* Left alignment */
    {
        ...
        temph =
          (uint16_t)((uint16_t)((uint16_t)templ << 6) | (uint16_t)(temph << 8));
    }
...
FlagStatus ADC1_GetAWDChannelStatus(ADC1_Channel_TypeDef Channel)
{
    ...
    {
        status = (uint8_t)(ADC1->AWSRH &
                           (uint8_t)((uint8_t)1 << (Channel - (uint8_t)8)));
    }
...
FlagStatus ADC1_GetFlagStatus(ADC1_Flag_TypeDef Flag)
{
    ...
    if ((Flag & 0x0F) == 0x01) {
        ...
        if (temp < 8) {
            ...
            flagstatus =
              (uint8_t)(ADC1->AWSRH & (uint8_t)((uint8_t)1 << (temp - 8)));
        }
...
ITStatus ADC1_GetITStatus(ADC1_IT_TypeDef ITPendingBit)
{
    ...
    if (((uint16_t)ITPendingBit & 0xF0) == 0x10) {
        ...
        if (temp < 8) {
            ...
            itstatus =
              (ITStatus)(ADC1->AWSRH & (uint8_t)((uint8_t)1 << (temp - 8)));
        }
    } else /* Get EOC | AWD flag status */
    {
