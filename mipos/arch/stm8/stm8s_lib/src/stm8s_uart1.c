...
void UART1_DeInit(void)
{
    ...
    UART1->BRR2 =
      UART1_BRR2_RESET_VALUE; /* Set UART1_BRR2 to reset value 0x00 */
    UART1->BRR1 =
      UART1_BRR1_RESET_VALUE; /* Set UART1_BRR1 to reset value 0x00 */
                ...
                UART1_Mode_TypeDef Mode)
{
    ...
    /* Set the UART1 BaudRates in BRR1 and BRR2 registers according to
     * UART1_BaudRate value */
    BaudRate_Mantissa = ((uint32_t)CLK_GetClockFreq() / (BaudRate << 4));
    BaudRate_Mantissa100 =
      (((uint32_t)CLK_GetClockFreq() * 100) / (BaudRate << 4));
    /* Set the fraction of UART1DIV  */
    UART1->BRR2 |=
      (uint8_t)((uint8_t)(((BaudRate_Mantissa100 - (BaudRate_Mantissa * 100))
                           << 4) /
                          100) &
                (uint8_t)0x0F);
    ...
    /* Disable the Transmitter and Receiver before setting the LBCL, CPOL and
     * CPHA bits */
    ...
    UART1->CR3 |=
      (uint8_t)((uint8_t)SyncMode &
                (uint8_t)(UART1_CR3_CPOL | UART1_CR3_CPHA | UART1_CR3_LBCL));
 ...
 * @param  UART1_IT specifies the USART interrupt sources to be enabled or
 ...
 *         - UART1_IT_RXNE_OR: Receive Data register not empty and Overrun
 * interrupt
...
void UART1_ITConfig(UART1_IT_TypeDef UART1_IT, FunctionalState NewState)
{
    ...
    itpos =
      (uint8_t)((uint8_t)1 << (uint8_t)((uint8_t)UART1_IT & (uint8_t)0x0F));
...
void UART1_HalfDuplexCmd(FunctionalState NewState)
{
    ...
    if (NewState != DISABLE) {
        ...
        UART1->CR5 &=
          (uint8_t)~UART1_CR5_HDSEL; /**< UART1 Half Duplex Disable */
    }
}
 ...
 * @param  UART1_LINBreakDetectionLength specifies the LIN break detection
 * length. This parameter can be any of the
...
{
    assert_param(
      IS_UART1_LINBREAKDETECTIONLENGTH_OK(UART1_LINBreakDetectionLength));
 ...
 * @note   This function is valid only for UART1 because is related to SmartCard
 * mode.
 * @param  NewState: new state of the Smart Card mode.
 *         This parameter can be: ENABLE or DISABLE.
 * @retval None
...
void UART1_SmartCardNACKCmd(FunctionalState NewState)
{
    ...
    if (NewState != DISABLE) {
        /* Enable the NACK transmission by setting the NACK bit in the CR5
         * register */
        UART1->CR5 |= UART1_CR5_NACK;
    } else {
        /* Disable the NACK transmission by clearing the NACK bit in the CR5
         * register */
...
void UART1_ReceiverWakeUpCmd(FunctionalState NewState)
{
    ...
    if (NewState != DISABLE) {
        ...
        /* Disable the mute mode UART1 by clearing the RWU bit in the CR1
         * register */
...
uint16_t UART1_ReceiveData9(void)
{
    ...
    temp =
      (uint16_t)(((uint16_t)((uint16_t)UART1->CR1 & (uint16_t)UART1_CR1_R8))
                 << 1);
 ...
 *         The clock source is divided by the value given in the register (8
 * bits)
 *         - 0000 0000 Reserved
 ...
 *         operation to UART1_DR register (UART1_SendData8() or
 * UART1_SendData9()).
 *
 ...
 *           cleared by software sequence: a read operation to UART1_SR register
 *           (UART1_GetITStatus()) followed by a read operation to UART1_DR
 * register (UART1_ReceiveData8() or UART1_ReceiveData9()).
 *
 *         - RXNE pending bit can be also cleared by a read to the UART1_DR
 * register (UART1_ReceiveData8() or UART1_ReceiveData9()).
 *
 *         - TC (Transmit complete) pending bit can be cleared by software
 *           sequence: a read operation to UART1_SR register
 * (UART1_GetITStatus()) followed by a write operation to UART1_DR register
 * (UART1_SendData8() or UART1_SendData9()).
 *
