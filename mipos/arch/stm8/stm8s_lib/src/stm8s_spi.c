...
/**
 * @brief  Deinitializes the SPI peripheral registers to their default reset
 * values.
 * @param  None
 * @retval None
 */
              ...
              uint8_t CRCPolynomial)
{
    ...
    SPI->CR2 =
      (uint8_t)((uint8_t)(Data_Direction) | (uint8_t)(Slave_Management));
 ...
 * @param  SPI_IT Specifies the SPI interrupts sources to be enabled or
 * disabled.
 ...
 * @param  NewState Indicates the new state of the SPI Software slave
 * management. This parameter can be: ENABLE or DISABLE.
 * @retval None
 */
void SPI_NSSInternalSoftwareCmd(FunctionalState NewState)
{
    ...
    if (NewState != DISABLE) {
        ...
        SPI->CR2 &=
          (uint8_t)(~SPI_CR2_SSI); /* Reset NSS pin internally by software*/
    }
}
 ...
 * @param  SPI_Direction Specifies the data transfer direction in bi-directional
 * mode.
 * @retval None
 */
 ...
 * sequence: a read/write operation to SPI_SR register (SPI_GetFlagStatus())
 * followed by a write operation to SPI_CR1 register (SPI_Cmd() to enable the
 * SPI).
 * @retval None
 */
 ...
 *         - MODF (Mode Fault) interrupt pending bit is cleared by software
 * sequence: a read/write operation to SPI_SR register (SPI_GetITStatus())
 * followed by a write operation to SPI_CR1 register (SPI_Cmd() to enable the
 * SPI).
 * @retval None
 */
void SPI_ClearITPendingBit(SPI_IT_TypeDef SPI_IT)
{
    ...
    itpos = (uint8_t)((uint8_t)1
                      << (uint8_t)((uint8_t)(SPI_IT & (uint8_t)0xF0) >> 4));
