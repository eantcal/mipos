...
void UART3_DeInit(void)
{
    ...
    UART3->BRR2 =
      UART3_BRR2_RESET_VALUE; /*Set UART3_BRR2 to reset value 0x00 */
    UART3->BRR1 =
      UART3_BRR1_RESET_VALUE; /*Set UART3_BRR1 to reset value 0x00 */
                ...
                UART3_Mode_TypeDef Mode)
{
    ...
    /* Set the UART3 BaudRates in BRR1 and BRR2 registers according to
     * UART3_BaudRate value */
 ...
 * @param  UART3_IT specifies the UART3 interrupt sources to be enabled or
 ...
 *         - UART3_IT_RXNE_OR: Receive Data register not empty/Over run error
 * interrupt
...
void UART3_ITConfig(UART3_IT_TypeDef UART3_IT, FunctionalState NewState)
{
    ...
    itpos =
      (uint8_t)((uint8_t)1 << (uint8_t)((uint8_t)UART3_IT & (uint8_t)0x0F));
 ...
 * @param  UART3_LINBreakDetectionLength specifies the LIN break detection
  ...
  UART3_LINBreakDetectionLength_TypeDef UART3_LINBreakDetectionLength)
{
    /* Check the parameters */
    assert_param(
      IS_UART3_LINBREAKDETECTIONLENGTH_OK(UART3_LINBreakDetectionLength));
 ...
 *         This parameter can be any of the @ref UART3_LinAutosync_TypeDef
 * values.
...
void UART3_ReceiverWakeUpCmd(FunctionalState NewState)
{
    ...
    if (NewState != DISABLE) {
        ...
        /* Disable the mute mode UART3 by clearing the RWU bit in the CR1
         * register */
...
uint16_t UART3_ReceiveData9(void)
{
    ...
    temp =
      (uint16_t)(((uint16_t)((uint16_t)UART3->CR1 & (uint16_t)UART3_CR1_R8))
                 << 1);
 ...
 *           operation to UART3_DR register (UART3_SendData8() or
 * UART3_SendData9()).
 *
 ...
 *           (UART3_GetITStatus()) followed by a read operation to UART3_DR
 * register (UART3_ReceiveData8() or UART3_ReceiveData9()).
 *
 *         - RXNE pending bit can be also cleared by a read to the UART3_DR
 * register (UART3_ReceiveData8() or UART3_ReceiveData9() ).
 *
 *         - TC (Transmit complete) pending bit can be cleared by software
 *           sequence: a read operation to UART3_SR register
 * (UART3_GetITStatus()) followed by a write operation to UART3_DR register
 *           (UART3_SendData8()or UART3_SendData9()).
 *
