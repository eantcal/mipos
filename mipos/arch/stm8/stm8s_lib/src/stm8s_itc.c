...
uint8_t ITC_GetCPUCC(void)
{
    ...
    __asm__("push cc");
    __asm__("pop a"); /* Ignore compiler warning, the returned value is in A
                         register */
#else                      /* _IAR_ */
    asm("push cc");
    asm("pop a"); /* Ignore compiler warning, the returned value is in A
                     register */
#endif                     /* _COSMIC_*/
}
#ifdef _SDCC_
...
ITC_PriorityLevel_TypeDef ITC_GetSoftwarePriority(ITC_Irq_TypeDef IrqNum)
{
    ...
    switch (IrqNum) {
        case ITC_IRQ_TLI: /* TLI software priority can be read but has no
                             meaning */
        case ITC_IRQ_AWU:
        case ITC_IRQ_CLK:
        case ITC_IRQ_PORTA:
  ...
  defined(STM8AF622x)
        case ITC_IRQ_ADC1:
#endif /*STM8S105, STM8S005, STM8S103 or STM8S003 or STM8S903 or STM8AF626x or \
          STM8AF622x */
#if defined(STM8S903) || defined(STM8AF622x)
                             ...
                             ITC_PriorityLevel_TypeDef PriorityValue)
{
    ...
    switch (IrqNum) {
        case ITC_IRQ_TLI: /* TLI software priority can be written but has no
                             meaning */
        case ITC_IRQ_AWU:
        case ITC_IRQ_CLK:
        case ITC_IRQ_PORTA:
