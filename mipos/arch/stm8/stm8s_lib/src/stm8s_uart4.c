...
void UART4_DeInit(void)
{
    ...
    UART4->BRR2 =
      UART4_BRR2_RESET_VALUE; /*  Set UART4_BRR2 to reset value 0x00 */
    UART4->BRR1 =
      UART4_BRR1_RESET_VALUE; /*  Set UART4_BRR1 to reset value 0x00 */
                ...
                UART4_Mode_TypeDef Mode)
{
    ...
    /* Set the UART4 BaudRates in BRR1 and BRR2 registers according to
     * UART4_BaudRate value */
    BaudRate_Mantissa = ((uint32_t)CLK_GetClockFreq() / (BaudRate << 4));
    BaudRate_Mantissa100 =
      (((uint32_t)CLK_GetClockFreq() * 100) / (BaudRate << 4));

    /* The fraction and MSB mantissa should be loaded in one step in the BRR2
     * register*/
    ...
    UART4->CR3 |=
      (uint8_t)((uint8_t)SyncMode &
                (uint8_t)(UART4_CR3_CPOL | UART4_CR3_CPHA | UART4_CR3_LBCL));
 ...
 * @param  UART4_IT specifies the UART4 interrupt sources to be enabled or
 ...
 *         - UART4_IT_RXNE_OR: Receive Data register not empty/Over run error
 * interrupt
...
void UART4_ITConfig(UART4_IT_TypeDef UART4_IT, FunctionalState NewState)
{
    ...
    itpos =
      (uint8_t)((uint8_t)1 << (uint8_t)((uint8_t)UART4_IT & (uint8_t)0x0F));
...
void UART4_HalfDuplexCmd(FunctionalState NewState)
{
    ...
    if (NewState != DISABLE) {
        ...
        UART4->CR5 &=
          (uint8_t)~UART4_CR5_HDSEL; /**< UART4 Half Duplex Disable */
    }
}
 ...
 * @param  UART4_LINBreakDetectionLength specifies the LIN break detection
  ...
  UART4_LINBreakDetectionLength_TypeDef UART4_LINBreakDetectionLength)
{
    /* Check parameters */
    assert_param(
      IS_UART4_LINBREAKDETECTIONLENGTH_OK(UART4_LINBreakDetectionLength));
 ...
 *         This parameter can be any of the @ref UART4_LinAutosync_TypeDef
 * values.
...
void UART4_ReceiverWakeUpCmd(FunctionalState NewState)
{
    ...
    if (NewState != DISABLE) {
        ...
        /* Disable the mute mode UART4 by clearing the RWU bit in the CR1
         * register */
...
uint16_t UART4_ReceiveData9(void)
{
    ...
    temp =
      ((uint16_t)(((uint16_t)((uint16_t)UART4->CR1 & (uint16_t)UART4_CR1_R8))
                  << 1));
 ...
 *           operation to UART4_DR register (UART4_SendData8() or
 * UART4_SendData9()).
 *
 ...
 *           (UART4_GetITStatus()) followed by a read operation to UART4_DR
 * register (UART4_ReceiveData8() or UART4_ReceiveData9()).
 *
