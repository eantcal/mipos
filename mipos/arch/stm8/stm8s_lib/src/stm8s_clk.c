...
CONST uint8_t HSIDivFactor[4] = {
    1,
    2,
    4,
    8
}; /*!< Holds the different HSI Divider factors */
CONST uint8_t CLKPrescTable[8] = {
    1, 2, 4, 8, 10, 16, 20, 40
 ...
 * @note   Before using the LSI clock you have to enable the option bytes
 * (LSI_EN option bit is set).
 * @retval None
 */
 ...
 * @param   CLK_Peripheral : This parameter specifies the peripheral clock to
 * gate. This parameter can be any of the  @ref CLK_Peripheral_TypeDef
 * enumeration.
                               ...
                               FunctionalState NewState)
{
    ...
    if (((uint8_t)CLK_Peripheral & (uint8_t)0x10) == 0x00) {
        if (NewState != DISABLE) {
            ...
            CLK->PCKENR1 |= (uint8_t)((uint8_t)1 << ((uint8_t)CLK_Peripheral &
                                                     (uint8_t)0x0F));
        } else {
            /* Disable the peripheral Clock */
            CLK->PCKENR1 &= (uint8_t)(~(uint8_t)((
              (uint8_t)1 << ((uint8_t)CLK_Peripheral & (uint8_t)0x0F))));
        }
    } else {
        if (NewState != DISABLE) {
            /* Enable the peripheral Clock */
            CLK->PCKENR2 |= (uint8_t)((uint8_t)1 << ((uint8_t)CLK_Peripheral &
                                                     (uint8_t)0x0F));
        } else {
            /* Disable the peripheral Clock */
            CLK->PCKENR2 &= (uint8_t)(~(uint8_t)((
              (uint8_t)1 << ((uint8_t)CLK_Peripheral & (uint8_t)0x0F))));
        }
    }
}
  ...
  CLK_CurrentClockState_TypeDef CLK_CurrentClockState)
{
    ...
    if (Swif != ERROR) {
        ...
        if ((CLK_CurrentClockState == CLK_CURRENTCLOCKSTATE_DISABLE) &&
            (clock_master == CLK_SOURCE_HSI)) {
            CLK->ICKR &= (uint8_t)(~CLK_ICKR_HSIEN);
        } else if ((CLK_CurrentClockState == CLK_CURRENTCLOCKSTATE_DISABLE) &&
                   (clock_master == CLK_SOURCE_LSI)) {
            CLK->ICKR &= (uint8_t)(~CLK_ICKR_LSIEN);
        } else if ((CLK_CurrentClockState == CLK_CURRENTCLOCKSTATE_DISABLE) &&
                   (clock_master == CLK_SOURCE_HSE)) {
            CLK->ECKR &= (uint8_t)(~CLK_ECKR_HSEEN);
        }
    }
    return (Swif);
    ...
    if (NewState != DISABLE) {
        switch (CLK_IT) {
            case CLK_IT_SWIF: /* Enable the clock switch interrupt */
                CLK->SWCR |= CLK_SWCR_SWIEN;
                break;
            case CLK_IT_CSSD: /* Enable the clock security system detection
                                 interrupt */
    ...
    } else /*(NewState == DISABLE)*/
    {
        switch (CLK_IT) {
            case CLK_IT_SWIF: /* Disable the clock switch interrupt */
                CLK->SWCR &= (uint8_t)(~CLK_SWCR_SWIEN);
                break;
            case CLK_IT_CSSD: /* Disable the clock security system detection
                                 interrupt */
...
void CLK_SYSCLKConfig(CLK_Prescaler_TypeDef CLK_Prescaler)
{
    ...
    if (((uint8_t)CLK_Prescaler & (uint8_t)0x80) ==
        0x00) /* Bit7 = 0 means HSI divider */
    {
        CLK->CKDIVR &= (uint8_t)(~CLK_CKDIVR_HSIDIV);
        CLK->CKDIVR |=
          (uint8_t)((uint8_t)CLK_Prescaler & (uint8_t)CLK_CKDIVR_HSIDIV);
    } else /* Bit7 = 1 means CPU divider */
    {
        CLK->CKDIVR &= (uint8_t)(~CLK_CKDIVR_CPUDIV);
        CLK->CKDIVR |=
          (uint8_t)((uint8_t)CLK_Prescaler & (uint8_t)CLK_CKDIVR_CPUDIV);
    }
}
  ...
  CLK_HSITrimValue_TypeDef CLK_HSICalibrationValue)
{
    ...
    /* Store the new value */
    CLK->HSITRIMR =
      (uint8_t)((uint8_t)(CLK->HSITRIMR & (uint8_t)(~CLK_HSITRIMR_HSITRIM)) |
 ...
 * @par Full description:
 * This function reset SWBSY flag in order to reset clock switch operations
 * (target oscillator is broken, stabilization is longing too much, etc.).  If
 * at the same time \n software attempts to set SWEN and clear SWBSY, SWBSY
