...
#define KERNEL__PANIC(_MSG) \
  do { printk(_MSG); while (1); } while(0)
 ...
 *  Apply to: NON-RT TASK, RT-TASK
 */
#define mipos_t_resume(_TID) \
  mipos_t_notify_signal(_TID, SIGWKP)
  ...
  *  Apply to: NON-RT TASK
  */
#define mipos_t_suspend() \
  _mipos_t_suspend(SIGWKP)
   ...
   */
#define mipos_t_restart(_TID) do { \
   mipos_init_cs();\
   mipos_enter_cs();\
     mipos_task_t * p_task = _mipos_get_p_task(_TID);\
     if (p_task && p_task->entry_point && \
    ...
    */
#define mipos_t_restart_with_param(_TID, _NEW_PARAM) do { \
   mipos_init_cs();\
   mipos_enter_cs();\
     mipos_task_t * p_task = _mipos_get_p_task(_TID);\
     if (p_task && p_task->entry_point && \
...
// Reserved for internal use only
#define _mipos_t_set_wait_for_signal(_SIGNUM) do {\
  KERNEL_ENV.task_context_ptr->signal_waiting |= (_SIGNUM);\
 ...
 *
 */
#define mipos_t_notify_signal(_TID, _SIGNUM) do {\
  mipos_task_t * p_task;\
  mipos_init_cs();\
  mipos_enter_cs();\
