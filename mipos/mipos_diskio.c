...
int mipos_add_disk_driver(const mipos_disk_t* drv_ptr)
{
    ...
    memcpy(
      &_mipos_disk_drv[_mipos_disk_drv_cnt - 1], drv_ptr, sizeof(mipos_disk_t));
...
static mipos_disk_t* mipos_find_disk_driver(mipos_pdrv_t pdrv)
{
...
{
    mipos_disk_t const* drv_ptr = mipos_find_disk_driver(pdrv);

    return drv_ptr ? drv_ptr->_disk_status(pdrv) : MIPOS_DISK_STS_NOINIT;
}
...
int mipos_disk_init(mipos_pdrv_t pdrv)
{
    ...
    return drv_ptr ? drv_ptr->_disk_init(pdrv) : MIPOS_DISK_STS_NOINIT;
}
                    ...
                    mipos_sec_t count)
{
    ...
    return drv_ptr ? drv_ptr->_disk_read(pdrv, buf, sec, count)
                     ...
                     mipos_sec_t count)
{
    ...
    return drv_ptr ? drv_ptr->_disk_write(pdrv, buf, sec, count)
                   : MIPOS_DISK_RES_NOTRDY;
}
...
int mipos_disk_ioctl(mipos_pdrv_t pdrv, mipos_ioctl_cmd_t cmd, void* buf)
{
    ...
    return drv_ptr ? drv_ptr->_disk_ioctl(pdrv, cmd, buf)
