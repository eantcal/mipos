...
static mipos_console_cmd_t _mipos_cmd_list[] = {
...
{
    if ((c == end_line_char) && (console_flags & CONSOLE_TX_CRLF)) {
...
static void show_command_list(const mipos_console_cmd_t* cmd_list)
{
...
static int try_exec_cmd(int argc,
                        char* argv[],
                        const mipos_console_cmd_t* cmd_list)
{
    int index_of_cmd = 0;
 ...
 */
static int recvr_task(task_param_t context)
{
    unsigned char c = 0;
    ...
    while (1) {
        ...
        if (c) {
            ...
            if ((c == (unsigned char)end_line_char)) {
                ...
                argc = mipos_split_command(
                  cmd_str, cmd_len, argv, sizeof(argv) / sizeof(argv[0]));

                if (mipos_cmd_list &&
                    try_exec_cmd(argc, argv, mipos_cmd_list) >= 0) {
                    ;
                } else if (registered_cmd_list &&
                           try_exec_cmd(argc, argv, registered_cmd_list) >= 0) {
                    ;
                }
 ...
 * @return none
 */
static int sender_task(task_param_t context)
{
    mipos_q_item_t item;
    mipos_puts("\r\n");
    ...
    while (1) {
        static unsigned char const* buffer;
        unsigned int len;
...
#ifdef MIPOS64
        sprintf(local_buffer, "%p ", (void*)&b[buf_index]);
#else
        sprintf(
          local_buffer, "0x%08X ", (mipos_reg_t)((const int*)&b[buf_index]));
#endif
...
static int exec_ps_command(int argc, char* argv[])
{
    ...
    for (id = 1 /* skip idle*/; id < MIPOS_MAX_TASKS_COUNT; ++id) {
...
#if defined(MIPOS64) || defined(MIPOS32)
            sprintf(tx_buf, "%p ", p_task->entry_point);
#else
            sprintf(tx_buf, "%04x ", (uint32_t)p_task->entry_point);
#endif
            mipos_puts(tx_buf);

            sprintf(tx_buf, "psig:%08x ", p_task->signal_pending);
            mipos_puts(tx_buf);

            sprintf(tx_buf, "wsig:%08x ", p_task->signal_waiting);
            mipos_puts(tx_buf);
...
#if defined(MIPOS64) || defined(MIPOS32)
            sprintf(tx_buf, "Stack:%p ", top_of_stack);
#else
            sprintf(tx_buf, "Stack:%04x ", (unsigned int)top_of_stack);
#endif
            mipos_puts(tx_buf);

#if defined(MIPOS64) || defined(MIPOS32)
            sprintf(tx_buf, "SP=%p ", (void*)p_task->process_stack_pointer);
#else
            sprintf(tx_buf, "SP=%04x ", p_task->process_stack_pointer);
#endif
            mipos_puts(tx_buf);
