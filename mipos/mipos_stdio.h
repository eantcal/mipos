...
#ifndef __mipos_STDIO_H__
#define __mipos_STDIO_H__
...
typedef struct _mipos_FILE {
    mipos_fs_file_handle_t _handle;
    bool_t              _read_only;
} 
mipos_FILE;
 ...
 *  by the mipos_FILE object whose pointer is returned. 
 ...
 *         new empty file.<cr>
 *
 * @return a pointer to a mipos_FILE object if the file has been succesfully 
 *         opened. A mipos_FILE object is used to identify the stream on all further 
 *         operations involving it. Otherwise, a null pointer is returned.
 */
mipos_FILE* mipos_fopen(const char * filename, const char * mode);
 ...
 *
 * @param stream: pointer to a mipos_FILE object that identifies the stream.
 *
 ...
 */
int mipos_feof(mipos_FILE * stream);
 ...
 * @param stream: Pointer to a mipos_FILE object that specifies an input stream. 
 *
 ...
 *  The postion indicator of the stream is advanced by the total amount of 
 *  bytes written. The total amount of bytes written is (size * count).
 *
 * @param ptr:Pointer to the array of elements to be written
 * @param size: Size in bytes of each element to be written.
 * @param count: Number of elements, each one with a size of size bytes.
 * @param stream: Pointer to a mipos_FILE object wich specifies an output stream
 *
 ...
 * indicates an error.
 */
size_t mipos_fwrite(const void * ptr, 
                    size_t size, 
                    size_t count, 
                    mipos_FILE * stream);
 ...
 */
int mipos_fseek(mipos_FILE * stream,long offset,int origin);
 ...
 *  from the beginning of the file. For text streams, the value is not guaranteed 
 *  to be the exact number of bytes from the beginning of the file, 
 *  but the value returned can still be used to restore the position indicator 
 *  to this position using mipos_fseek.
 *
 * @param Pointer to a mipos_FILE object that identifies the stream.
 *
 * @return On success, the current value of the position 
 *         indicator is returned.if an error occurs,-1L is returned.
 */
long mipos_ftell(mipos_FILE * stream);
 ...
 *         function returns EOF and the corresponding error or eof indicator is set. 
 *         You can use either ferror or mipos_feof to determine whether an 
