...
added /  to the project. The working buffer occupies (FF_MAX_LFN + 1) * 2 bytes
and /  additional 608 bytes at exFAT enabled. FF_MAX_LFN can be in range from 12
...
/  when LFN is enabled. Also behavior of string I/O functions will be affected
by /  this option. When LFN is not enabled, this option has no effect.
*/
...
/  When FF_STR_VOLUME_ID is set to 1, also pre-defined strings can be used as
drive /  number in the path name. FF_VOLUME_STRS defines the drive ID strings
for each /  logical drives. Number of items must be equal to FF_VOLUMES. Valid
...
to disable /  the timestamp function. All objects modified by FatFs will have a
...
options have no effect at read-only configuration (FF_FS_READONLY = 1). */
